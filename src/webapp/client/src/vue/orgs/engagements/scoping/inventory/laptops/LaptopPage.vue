        ...
        <template v-slot:content>
            <v-list-item class="px-0">
                ...
                <v-list-item-action>
                    ...
                    >
                        <confirmation-dialog
                            :confirmation="laptopName"
                            :in-progress="deleteInProgress"
                            @do-cancel="showHideDelete=false"
                            @do-confirm="onDeleteLaptop"
                        >
                            <p>
                                You are about to delete the laptop <b>{{ laptopName }}</b>.
...
export default class LaptopPage extends Vue {
    showHideDelete : boolean = false
    deleteInProgress: boolean = false

    get isLoading() : boolean {
        return !this.currentOrg || !this.currentEngagement || !this.currentLaptop
    }
    ...
    get laptopName() : string {
        if (!this.currentLaptop) {
            return ''
        }
        return this.currentLaptop.Inventory.Name
    }

    get permissionsForDelete() : Permission[] {
        return [Permission.PLaptopsDelete]
    }
    ...
    get commentPermissions() : Permission[] {
        return [Permission.PLaptopsView, Permission.PCommentsList, Permission.POrgUsersView]
    }

    onDeleteLaptop() {
        this.deleteInProgress = true
        GrchiveApi.inventory.deleteInventory(InventoryType.ITLaptop, this.currentOrg!.Id, this.currentEngagement!.Id, this.currentLaptop!.Id).then(() => {
            this.$router.replace({
                name: 'scopingLaptops',
                params: this.$route.params,
            })
            ...
            ErrorHandler.failurePopupOnError(err, {
                context: 'Failed to delete laptop.'
            })
        }).finally(() => {
    ...
    refreshData() {
        ...
        const laptopId : number = Number(this.$route.params.laptopId)
