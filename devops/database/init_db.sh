...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname grchive <<-EOSQL
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA public
        GRANT EXECUTE ON FUNCTIONS TO $POSTGRES_WEBAPP_USER;

    CREATE SCHEMA audit AUTHORIZATION $POSTGRES_USER;
    GRANT USAGE ON SCHEMA audit TO $POSTGRES_WEBAPP_USER;
    ALTER DEFAULT PRIVILEGES IN SCHEMA audit
        GRANT SELECT, INSERT ON TABLES TO $POSTGRES_WEBAPP_USER;
